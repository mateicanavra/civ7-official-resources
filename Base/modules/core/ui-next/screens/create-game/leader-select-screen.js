...
import { createMemo, createSelector, createSignal, onMount, createComponent, Show, For, createRenderEffect, createEffect, on } from '../../../vendor/solid-js/dist/solid.js';
...
const LeaderSelectLeadersPanel = () => {
  ...
  const showSearch = createMemo(() => !isSmallScreen() && !IsControllerActive() && isDesktop());
  ...
  const audio = useAudio("LeaderSelectScreen");
  onMount(() => {
    audio("popup-open");
  });
  return (() => {
...
const LeaderSelectScreenComponent = () => {
  const leaderModel = LeaderSelectModel.get();
  const [debouncedLeader, setLeader, isLeaderChanging] = createDebouncedSignal(60, leaderModel.selectedLeader());
