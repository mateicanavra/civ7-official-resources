...
class SaveLoadModel {
  ...
  updateSavesWithFileQueryEntries(fileList) {
    ...
    fileList.forEach((file) => {
      ...
      const visitedMods = /* @__PURE__ */ new Set();
      if (mods2.length > 0) {
        for (const mod of mods2) {
          let entry = "";
          const modID = mod.ID;
          const modHandle = Modding.getModHandle(mod.ID);
          const modInfo = modHandle != null ? installedMods.find((installedMod) => {
            return installedMod.handle == modHandle;
          }) : null;
          const shouldSkip = modHandle != null && visitedMods.has(modHandle);
          const shouldIgnore = ignoreMods.has(modID);
          const inDebugBuild = BuildInfo.build == "Debug";
          if (!(shouldSkip || shouldIgnore) || inDebugBuild) {
            if (modHandle) {
              visitedMods.add(modHandle);
            }
            if (shouldIgnore) {
              entry += "(DEBUG) ";
            }
      ...
      saveGameInfos.push({
        ...
        civIconUrl: file.hostCivilization ? "blp:civ_sym_" + file.hostCivilization.slice(13).toLowerCase() : "blp:civ_sym_unknown",
...
export { DEFAULT_SAVE_GAME_INFO, DeleteDoneEvent, DeleteDoneEventName, LoadCompleteEvent, LoadCompleteEventName, QueryCompleteEvent, QueryCompleteEventName, QueryDoneEvent, QueryDoneEventName, QuickSaveDoneEvent, QuickSaveDoneEventName, ResolveConflictDoneEvent, ResolveConflictDoneEventName, SaveDoneEvent, SaveDoneEventName, SyncDoneEvent, SyncDoneEventName, SaveLoadData as default };
//# sourceMappingURL=model-save-load.js.map
