...
var MPLobbySlotActionType = /* @__PURE__ */ ((MPLobbySlotActionType2) => {
  ...
  MPLobbySlotActionType2["HUMAN"] = "SLOT_ACTION_TYPE_HUMAN";
...
class MPLobbyDropdownOptionData {
  ...
  get serializedItemList() {
    ...
    return stringifyJSON(this.itemList);
  }
}
...
class MPLobbyDataModel {
  ...
  checkHotseatHumanShowCheckCallback = (playerID, actionOption) => {
    return Configuration.getGame().isHotseat && this.canChangeSlotStatus(playerID, actionOption);
  };
  ...
  slotActionsData = /* @__PURE__ */ new Map([
    [
      "SLOT_ACTION_TYPE_HUMAN" /* HUMAN */,
      {
        actionType: "SLOT_ACTION_TYPE_HUMAN" /* HUMAN */,
        displayName: Locale.compose("LOC_SLOT_ACTION_HOTSEAT_HUMAN"),
        showCheckCallback: this.checkHotseatHumanShowCheckCallback,
        slotStatus: SlotStatus.SS_TAKEN
      }
    ],
    [
   ...
   */
  get isReadyOptionHidden() {
    return Configuration.getGame().isHotseat;
  }
  /**
   * Should the column header that gives the ability to "Kick" players from the match be hidden?
   */
  get isKickOptionHidden() {
    return !Configuration.getGame().isKickVoting && Network.getHostPlayerId() != GameContext.localPlayerID || Configuration.getGame().isHotseat;
  }
    ...
    if (MultiplayerShellManager.unitTestMP) {
      ...
      for (let curPlayerID = 0; curPlayerID < numPlayers; curPlayerID++) {
        ...
        if (isHost) {
          ...
          statusIcon = "blp:mpicon_localplayer.png";
        ...
        const playerInfoDropdown = this.createSlotActionsDropdown(
          curPlayerID,
          gamertag,
        ...
        if (playerConfig.isParticipant) {
          ...
          playerConfig.isHuman && Configuration.getGame().isHotseat);
  ...
  getCivilizationTooltip(civilizationType, playerID) {
    ...
    if (tt == null) {
      ...
      const isHigherRes = window.innerHeight >= 1080;
      tt = `
			...
			${isHigherRes ? civData.bonuses && civData.bonuses.length > 0 ? `[N][STYLE:text-secondary][STYLE:font-title-base]${Locale.compose("LOC_CREATE_CIV_UNIQUE_BONUSES_SUBTITLE")}[/S][/S]
      ...
      }).join("[N]")}` : "" : civData.bonuses && civData.bonuses.length > 0 ? `[N] [N][STYLE:text-secondary][STYLE:font-title-base]${Locale.compose("LOC_CREATE_CIV_UNIQUE_BONUSES_SUBTITLE")}[/S][/S]
				[N]${civData.bonuses.map((bonus) => {
        return `[icon:${bonus.icon}] [STYLE:text-secondary][B]${Locale.compose(bonus.title)}[/B][/S]`;
      }).join("[N]")}` : ""}
			`;
    }
  ...
  getLeaderTooltip(leaderType) {
    ...
    if (tt == null) {
      ...
      tt = `
			...
			${leaderData.abilityText ? `[N]${Locale.compose(leaderData.abilityText)}` : ""}
			`;
  ...
  getReadyStatus(localPlayerId) {
    ...
    if (Network.isPlayerStartReady(localPlayerId)) {
      if (Configuration.getGame().isHotseat) {
        return "STARTING_GAME" /* STARTING_GAME */;
      }
  ...
  createSlotActionsDropdown(playerID, gamertag, isDisabled, lobbyPlayerConnectionStatus) {
    ...
    dropdownData.isDisabled = isDisabled;
    dropdownData.tooltip = lobbyPlayerConnectionStatus == 1 /* DISCONNECTED */ && !Configuration.getGame().isHotseat ? Locale.compose("LOC_UI_MP_NOT_CONNECTED_TO_LOBBY") : void 0;
    return dropdownData;
  }
  onGameReady() {
    Network.toggleLocalPlayerStartReady();
  ...
  onSlotActionDropdown(event) {
    ...
    if (playerIDStr) {
      ...
      switch (slotActionType) {
        ...
        case "SLOT_ACTION_TYPE_HUMAN" /* HUMAN */:
          {
            ...
            if (slotActionData && slotActionData.slotStatus != void 0) {
              ...
              if (playerConfig) {
                ...
                if (slotActionData.slotStatus == SlotStatus.SS_COMPUTER || slotActionData.slotStatus == SlotStatus.SS_TAKEN) {
  ...
  canChangeSlotStatus(targetPlayerID, slotActionData) {
    ...
    if (!Configuration.getGame().isHotseat && targetPlayerConfig.isHuman) {
      return false;
    }
    if (Configuration.getGame().isHotseat) {
...
export { LobbyUpdateEvent, LobbyUpdateEventName, MPLobbyDataModel, MPLobbyDataModelProxy, MPLobbyDropdownOptionData, MPLobbyReadyStatus, PlayerParamDropdownTypes, SMALL_SCREEN_MODE_MAX_HEIGHT, SMALL_SCREEN_MODE_MAX_WIDTH, instance as default };
//# sourceMappingURL=model-mp-staging-new.js.map
