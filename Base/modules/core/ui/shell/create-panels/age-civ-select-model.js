...
import { DatabaseCache } from '../../utilities/utilities-data.js';
...
function GetCivilizationData(Stylize = true) {
  ...
  const currentAgeType = GameSetup.findGameParameter("Age")?.value.value?.toString();
  if (playerCivilizations) {
    ...
    for (const civData of playerCivilizations.domain.possibleValues ?? []) {
      ...
      const civItems = civItemData.filter((item) => item.CivilizationType == civID);
      let civAbility;
      const civAbilities = civItems?.filter((item) => item.Kind == "KIND_TRAIT");
      civAbilities?.forEach((trait) => {
        if (!civAbility || trait.AgeType == currentAgeType) {
          civAbility = trait;
        }
      });
      ...
      results.push({
        ...
        description: Stylize ? Locale.stylize(civAbility?.Description ?? "") : civAbility?.Description ?? "",
        icon: icon ?? "",
        image: image ?? "",
        abilityTitle: Stylize ? Locale.stylize(civAbility?.Name ?? "") : civAbility?.Name ?? "",
        abilityText: Stylize ? Locale.stylize(civAbility?.Description ?? "") : civAbility?.Description ?? "",
        bonuses: bonusItems,
        tags,
        unlocks,
        unlockedBy,
...
export { GetAgeMap, GetCivilizationData };
//# sourceMappingURL=age-civ-select-model.js.map
