...
class MultiplayerShellManagerSingleton {
  ...
  onGameBrowse(serverType, skipToGameCreator = false) {
    ...
    switch (serverType) {
      ...
      default:
        console.error(`mp-shell-logic: Unexpected server type '${serverType}'.`);
        return;
    }
    ...
    if (Network.getLocalHostingPlatform() == HostingType.HOSTING_TYPE_GAMECENTER && !Network.isChildOnlinePermissionsGranted()) {
      ...
      return;
    } else if (Network.isChildAccount() || Network.isTeenAccount()) {
  ...
  openChildMultiplayer() {
    if (this.waitingForParentalPermissions) {
      ...
      if (Network.isChildOnlinePermissionsGranted() || blockInfo.reason === BlockedAccessReason.NONE) {
        this.sendPremiumCheckRequest();
      } else {
        window.dispatchEvent(new MainMenuReturnEvent());
  ...
  onGameMode() {
    ...
    if (Network.isFullAccountLinked()) {
      if (Network.isChildAccount() || Network.isTeenAccount()) {
        Network.sendParentalStatusQuery();
        this.waitingForParentalPermissions = true;
      } else {
  ...
  exitMPGame(errorTitle, errorBody) {
    ...
    if (errorTitle != "" && errorBody != "" && this.exitGameErrorDialogBoxID == void 0) {
      ...
      if (!UI.isGameCenterNetworkBuild() && gameConfig.isInternetMultiplayer && ContextManager.getCurrentTarget()?.tagName != "SCREEN-MP-BROWSER") {
   ...
   */
  refreshGameCenterAuthentication() {
    if (Network.getLocalHostingPlatform() != HostingType.HOSTING_TYPE_GAMECENTER) {
      return;
    }
...
export { MultiplayerCreateAttemptEvent, MultiplayerCreateAttemptEventName, MultiplayerCreateCompleteEvent, MultiplayerCreateCompleteEventName, MultiplayerCreateFailEvent, MultiplayerCreateFailEventName, MultiplayerGameAbandonedEvent, MultiplayerGameAbandonedEventName, MultiplayerJoinCompleteEvent, MultiplayerJoinCompleteEventName, MultiplayerJoinFailEvent, MultiplayerJoinFailEventName, MultiplayerMatchMakeCompleteEvent, MultiplayerMatchMakeCompleteEventName, MultiplayerMatchMakeFailEvent, MultiplayerMatchMakeFailEventName, MultiplayerShellManager as default };
//# sourceMappingURL=mp-shell-logic.js.map
