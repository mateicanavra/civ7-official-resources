...
function allowExtraLargeFonts(height) {
  return height >= 1e3;
}
...
const onGfxCoreDropdownUpdate = ({ id, dropdownItems = [] }, value) => {
  switch (id) {
    ...
    case "option-gfx-res": {
      ...
      if (resolution.i != Options.graphicsOptions.resolution.i || resolution.j != Options.graphicsOptions.resolution.j) {
      ...
      if (!allowExtraLargeFonts(resolution.j)) {
        const scale = Configuration.getUser().uiFontScale;
        if (scale == FontScale.XLarge) {
          Configuration.getUser().setUiFontScale(FontScale.Large);
        }
      }
      break;
    }
...
const onTutorialLevelInit = (optionInfo) => {
  ...
  const isTutorialOn = value === 4;
  optionInfo.currentValue = isTutorialOn;
  advisorWarningsOption.isDisabled = optionInfo.currentValue;
};
const onTutorialLevelUpdate = (optionInfo, value) => {
  ...
  optionInfo.currentValue = value;
  const newTutorialLevel = value ? 4 : 2;
  userConfig.setTutorialLevel(newTutorialLevel);
  if (value) {
    advisorWarningsOption.isDisabled = true;
    advisorWarningsOption.selectedItemIndex = 1;
  } else {
    advisorWarningsOption.isDisabled = false;
  }
  advisorWarningsOption.forceRender?.();
...
const onAdvisorWarningsInit = (optionInfo) => {
  if (optionInfo.id != "option-advisorwarnings") {
    console.warn("The advisor warnings handler was called by some other option '" + optionInfo.id + "'");
    return;
  }
  const value = Configuration.getUser().tutorialLevel;
  const selectedAdvisorWarningsIndex = Math.min(value, 1);
  optionInfo.selectedItemIndex = selectedAdvisorWarningsIndex;
};
const onAdvisorWarningslUpdate = (optionInfo, value) => {
  if (optionInfo.id != "option-advisorwarnings") {
    console.warn("The advisor warnings handler was called by some other option '" + optionInfo.id + "'");
    return;
  }
  const userConfig = Configuration.getUser();
  const newTutorialLevel = value * 2;
  userConfig.setTutorialLevel(newTutorialLevel);
  optionInfo.selectedItemIndex = value;
  if (optionInfo.selectedItemIndex != optionInfo.originalValue) {
    Options.needReloadRefCount += 1;
  } else {
    if (Options.needReloadRefCount > 0) {
      Options.needReloadRefCount -= 1;
...
const PlayerCombatCameraPan_optionSet = "user";
const PlayerCombatCameraPan_optionType = "Interface";
const PlayerCombatCameraPan_optionName = "PlayerCombatCameraPan";
const onPlayerCombatCameraPanInit = (optionInfo) => {
  const value = UI.getOption(
    PlayerCombatCameraPan_optionSet,
    PlayerCombatCameraPan_optionType,
    PlayerCombatCameraPan_optionName
  );
  optionInfo.selectedItemIndex = value;
};
const onPlayerCombatCameraPanUpdate = (optionInfo, value) => {
  UI.setOption(
    PlayerCombatCameraPan_optionSet,
    PlayerCombatCameraPan_optionType,
    PlayerCombatCameraPan_optionName,
...
const onPlotTooltipDelayInit = (optionInfo) => {
  optionInfo.currentValue = Configuration.getUser().plotTooltipDelay;
  const adjustedValue = Math.round((optionInfo.currentValue * 1e-3 + Number.EPSILON) * 10) / 10;
  optionInfo.formattedValue = Locale.compose("LOC_OPTIONS_PLOT_TOOLTIP_DELAY_VALUE", adjustedValue);
};
const onPlotTooltipDelayUpdate = (optionInfo, value) => {
  Configuration.getUser().setPlotTooltipDelay(value);
  optionInfo.currentValue = Configuration.getUser().plotTooltipDelay;
  const adjustedValue = Math.round((optionInfo.currentValue * 1e-3 + Number.EPSILON) * 10) / 10;
  optionInfo.formattedValue = Locale.compose("LOC_OPTIONS_PLOT_TOOLTIP_DELAY_VALUE", adjustedValue);
};
const onTooltipAutolockEnabledInit = (optionInfo) => {
  const value = Configuration.getUser().tooltipAutolockEnabled;
  optionInfo.currentValue = value;
  tooltipAutolockSliderOption.isDisabled = !value;
};
const onTooltipAutolockEnabledUpdate = (optionInfo, value) => {
  Configuration.getUser().setTooltipAutolockEnabled(value);
  optionInfo.currentValue = value;
  tooltipAutolockSliderOption.isDisabled = !value;
  tooltipAutolockSliderOption.forceRender?.();
};
const onTooltipAutolockInit = (optionInfo) => {
  optionInfo.currentValue = Configuration.getUser().tooltipAutolock;
  const adjustedValue = Math.round((optionInfo.currentValue * 1e-3 + Number.EPSILON) * 10) / 10;
  optionInfo.formattedValue = Locale.compose("LOC_OPTIONS_TOOLTIP_AUTOLOCK_VALUE", adjustedValue);
};
const onTooltipAutolockUpdate = (optionInfo, value) => {
  const clampedValue = Math.max(0, Math.min(5e3, Math.round(value)));
  Configuration.getUser().setTooltipAutolock(clampedValue);
  optionInfo.currentValue = Configuration.getUser().tooltipAutolock;
  const adjustedValue = Math.round((optionInfo.currentValue * 1e-3 + Number.EPSILON) * 10) / 10;
  optionInfo.formattedValue = Locale.compose("LOC_OPTIONS_TOOLTIP_AUTOLOCK_VALUE", adjustedValue);
};
...
const onUiAutoScaleUpdate = (optionInfo, value) => {
  ...
  uiScaleAdjustmentSliderOption.isDisabled = !value;
  uiScaleAdjustmentSliderOption.forceRender?.();
};
const onUiLayoutInit = (optionInfo) => {
  const stretch = UI.getStretchMenusToFit();
  if (optionInfo.dropdownItems) {
    for (let index = 0; index < optionInfo.dropdownItems.length; ++index) {
      if (optionInfo.dropdownItems[index].stretch == stretch) {
        optionInfo.selectedItemIndex = index;
        break;
      }
    }
    optionInfo.currentValue = optionInfo.originalValue = stretch ? 1 : 0;
  }
};
const onUiLayoutUpdate = (optionInfo, value) => {
  const newStretch = (optionInfo.dropdownItems?.[value]).stretch;
  UI.setStretchMenusToFit(newStretch);
  optionInfo.currentValue = newStretch ? 1 : 0;
};
const onUiLayoutRestore = (optionInfo) => {
  if (optionInfo.currentValue != optionInfo.originalValue) {
    if (optionInfo.originalValue == void 0) {
      console.error("options.ts: onUiLayoutRestore() - originalValue was undefined!");
      return;
    }
    UI.setStretchMenusToFit(optionInfo.originalValue == 1 ? true : false);
    optionInfo.currentValue = optionInfo.originalValue;
  }
};
const onUiAutoScaleAdjustmentInit = (optionInfo) => {
  let value = UI.getOption("user", "Accessibility", "UIAutoScaleAdjustment") ?? 100;
...
const onUiAutoScaleAdjustmentUpdate = (optionInfo, value) => {
  value = Math.floor(value);
  optionInfo.currentValue = value;
  optionInfo.formattedValue = `${value}%`;
...
const onHUDhOffsetUpdate = (optionInfo, value) => {
  value = Math.floor(value);
  optionInfo.currentValue = value;
  optionInfo.formattedValue = `${value}%`;
  UI.setOption("user", "Accessibility", "HUDOffset", value);
...
const onAutoUnitCycleDelayInit = (optionInfo) => {
  optionInfo.currentValue = Configuration.getUser().autoUnitCycleDelay;
  const adjustedValue = Math.round((optionInfo.currentValue * 1e-3 + Number.EPSILON) * 10) / 10;
  optionInfo.formattedValue = Locale.compose("LOC_OPTIONS_AUTOUNITCYCLE_DELAY_VALUE", adjustedValue);
};
const onAutoUnitCycleDelayUpdate = (optionInfo, value) => {
  Configuration.getUser().setAutoUnitCycleDelay(value);
  optionInfo.currentValue = Configuration.getUser().autoUnitCycleDelay;
  const adjustedValue = Math.round((optionInfo.currentValue * 1e-3 + Number.EPSILON) * 10) / 10;
  optionInfo.formattedValue = Locale.compose("LOC_OPTIONS_AUTOUNITCYCLE_DELAY_VALUE", adjustedValue);
};
function supports1080pOrAbove(options) {
...
Options.addInitCallback(() => {
  ...
  const uiFontScaleRanges = [];
  if (isConsole) {
    uiFontScaleRanges.push(
    ...
    if (allowExtraLargeFonts(window.innerHeight)) {
      uiFontScaleRanges.push({ label: "LOC_OPTIONS_UI_SCALE_EXTRA_LARGE", scale: FontScale.XLarge });
    }
  }
  Options.addOption({
    category: CategoryType.Accessibility,
  ...
  if (supports1080pOrAbove(Options.supportedOptions) && UI.supportsHIDPI() && UI.canSetAutoScaling()) {
    uiScaleAdjustmentSliderOption = {
      category: CategoryType.Accessibility,
      group: "general",
      type: OptionType.Slider,
      id: "option-system-uiautoscaleadjustment",
      initListener: onUiAutoScaleAdjustmentInit,
      updateListener: onUiAutoScaleAdjustmentUpdate,
      label: "LOC_OPTIONS_UI_AUTO_SCALE_ADJUSTMENT",
      description: "LOC_OPTIONS_UI_AUTO_SCALE_ADJUSTMENT_DESCRIPTION",
      min: 50,
      max: 125,
      steps: 15
    };
    ...
    if (UI.supports(UISupportedFeature.AutoScaleAdjustment)) {
      Options.addOption(uiScaleAdjustmentSliderOption);
    }
  }
  if (UI.supports(UISupportedFeature.StretchLayoutToFit)) {
    const layoutItemList = [
      { label: "LOC_OPTIONS_UI_MENU_LAYOUT_FIXED", stretch: false },
      { label: "LOC_OPTIONS_UI_MENU_LAYOUT_STRETCH", stretch: true }
    ];
    Options.addOption({
      category: CategoryType.Accessibility,
      group: "general",
      type: OptionType.Dropdown,
      id: "option-system-uilayout",
      initListener: onUiLayoutInit,
      updateListener: onUiLayoutUpdate,
      restoreListener: onUiLayoutRestore,
      label: "LOC_OPTIONS_UI_MENU_LAYOUT",
      description: "LOC_OPTIONS_UI_MENU_LAYOUT_DESCRIPTION",
      dropdownItems: layoutItemList
    });
  }
  if (UI.supports(UISupportedFeature.HUDOffsets)) {
    Options.addOption({
      ...
      group: "general",
      type: OptionType.Slider,
      id: "option-hud-h-offset",
      initListener: onHUDhOffsetInit,
      updateListener: onHUDhOffsetUpdate,
      label: "LOC_OPTIONS_UI_MENU_HUD_H_OFFSET",
      description: "LOC_OPTIONS_UI_MENU_HUD_H_OFFSET_DESCRIPTION",
      min: 0,
      max: 100,
      steps: 10
    });
  ...
  Options.addOption({
    ...
    group: "general",
    type: OptionType.Slider,
    id: "option-general-autounitcycledelay",
    initListener: onAutoUnitCycleDelayInit,
    updateListener: onAutoUnitCycleDelayUpdate,
    label: "LOC_OPTIONS_AUTOUNITCYCLE_DELAY",
    description: "LOC_OPTIONS_AUTOUNITCYCLE_DELAY_DESCRIPTION",
    min: 0,
    max: 2e3,
    currentValue: -1,
    steps: 20
  ...
  const advisorWarningsItemList = [
    { label: "LOC_OPTIONS_ADVISOR_WARNINGS_1", tooltip: "LOC_OPTIONS_ADVISOR_WARNINGS_1_DESCRIPTION" },
    { label: "LOC_OPTIONS_ADVISOR_WARNINGS_2", tooltip: "LOC_OPTIONS_ADVISOR_WARNINGS_2_DESCRIPTION" }
  ];
  advisorWarningsOption = {
    category: CategoryType.Game,
    group: "general",
    type: OptionType.Dropdown,
    id: "option-advisorwarnings",
    initListener: onAdvisorWarningsInit,
    updateListener: onAdvisorWarningslUpdate,
    label: "LOC_OPTIONS_ADVISOR_WARNINGS_0",
    description: "LOC_OPTIONS_ADVISOR_WARNINGS_0_DESCRIPTION",
    dropdownItems: advisorWarningsItemList,
    isDisabled: Configuration.getUser().tutorialLevel === TutorialLevel.TutorialOn
    //Tutorial popopups are enabled in GameCore at TutorialLevel 4, which should prevent you from disabling advisor warnings
  };
  Options.addOption({
    category: CategoryType.Game,
    group: "general",
    ...
    label: "LOC_OPTIONS_GAME_TUTORIAL_2",
    description: "LOC_OPTIONS_TUTORIAL_DESCRIPTION",
    optionSet: "user",
    optionType: "Gameplay",
    optionName: "TutorialLevel"
  });
  Options.addOption(advisorWarningsOption);
  ...
  Options.addOption({
    ...
    group: "tooltip",
    type: OptionType.Slider,
    id: "option-system-plottooltipdelay",
    initListener: onPlotTooltipDelayInit,
    updateListener: onPlotTooltipDelayUpdate,
    label: "LOC_OPTIONS_PLOT_TOOLTIP_DELAY",
    description: "LOC_OPTIONS_PLOT_TOOLTIP_DELAY_DESCRIPTION",
    min: 0,
    max: 2e3,
    currentValue: -1,
    steps: 20
  ...
  if (UI.isMouseAvailable()) {
    tooltipAutolockSliderOption = {
      category: CategoryType.System,
      group: "tooltip",
      type: OptionType.Slider,
      id: "option-system-tooltipautolock",
      initListener: onTooltipAutolockInit,
      updateListener: onTooltipAutolockUpdate,
      label: "LOC_OPTIONS_TOOLTIP_AUTOLOCK_LABEL",
      description: "LOC_OPTIONS_TOOLTIP_AUTOLOCK_DESCRIPTION",
      min: 100,
      max: 5e3,
      currentValue: -1,
      steps: 49
    ...
    Options.addOption({
      ...
      group: "tooltip",
      type: OptionType.Checkbox,
      id: "option-system-tooltipautolockenabled",
      initListener: onTooltipAutolockEnabledInit,
      updateListener: onTooltipAutolockEnabledUpdate,
      label: "LOC_OPTIONS_TOOLTIP_AUTOLOCK_ENABLED_LABEL",
      description: "LOC_OPTIONS_TOOLTIP_AUTOLOCK_ENABLED_DESCRIPTION",
      currentValue: false,
      optionSet: "user",
      optionType: "Interface",
      optionName: "TooltipAutoLockEnabled"
    });
    Options.addOption(tooltipAutolockSliderOption);
  }
  Options.addOption({
    category: CategoryType.System,
    group: "general",
  ...
  const PlayerCombatCameraPanItemList = [
    {
      label: "LOC_OPTIONS_PLAYER_COMBAT_CAMERA_PAN_0",
      tooltip: "LOC_OPTIONS_PLAYER_COMBAT_CAMERA_PAN_0_DESCRIPTION"
    },
    {
      label: "LOC_OPTIONS_PLAYER_COMBAT_CAMERA_PAN_1",
      tooltip: "LOC_OPTIONS_PLAYER_COMBAT_CAMERA_PAN_1_DESCRIPTION"
    },
    {
      label: "LOC_OPTIONS_PLAYER_COMBAT_CAMERA_PAN_2",
      tooltip: "LOC_OPTIONS_PLAYER_COMBAT_CAMERA_PAN_2_DESCRIPTION"
    }
  ];
  Options.addOption({
    category: CategoryType.System,
    group: "camera",
    type: OptionType.Dropdown,
    id: "option-playercombatcamerapan",
    initListener: onPlayerCombatCameraPanInit,
    updateListener: onPlayerCombatCameraPanUpdate,
    label: "LOC_OPTIONS_PLAYER_COMBAT_CAMERA_PAN",
    description: "LOC_OPTIONS_PLAYER_COMBAT_CAMERA_PAN_DESCRIPTION",
    dropdownItems: PlayerCombatCameraPanItemList
  });
  if (UI.isMouseAvailable()) {
    Options.addOption({
      category: CategoryType.System,
