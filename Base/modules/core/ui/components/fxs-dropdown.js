...
class FxsDropdown extends FxsActivatable {
  isOpen = false;
  selectedIndex = -1;
  labelElement;
  highlightElement;
  ...
  dropdownElements = [];
  openArrowElement;
  scrollableElement = document.createElement("fxs-scrollable");
  dropdownItemSlot = document.createElement("fxs-vslot");
  scrollArea;
  noSelectionCaption = "LOC_UI_DROPDOWN_NO_SELECTION";
  selectionCaption = "";
  wasOpenedUp = false;
  onScrollWheelEventListener = this.onScrollWheel.bind(this);
  onActivateEventListener = this.onActivate.bind(this);
  onEngineInputEventListener = this.onEngineInput.bind(this);
  dropdownSlotItemFocusInListener = this.onDropdownSlotItemFocusIn.bind(this);
  dropdownSlotItemFocusOutListener = this.onDropdownSlotItemFocusOut.bind(this);
  onClickOutsideEventListener = this.onClickOutside.bind(this);
  onTouchOutsideEventListener = this.onTouchOutside.bind(this);
  onBlurEventListener = this.onBlur.bind(this);
  onResizeEventListener = this.onResizeEvent.bind(this);
  activeDeviceTypeListener = this.onActiveDeviceTypeChanged.bind(this);
  listeningForClickOutside = false;
  onActivate() {
    this.toggleOpen();
  }
  ...
  onScrollWheel(event) {
    if (event.target instanceof Node) {
      ...
      if (event.deltaY > 0 && newScrollPos >= 1 || event.deltaY < 0 && newScrollPos <= -1) {
  ...
  onDropdownSlotItemFocusOut(focusEvent) {
    ...
    if (target instanceof Node && this.Root.contains(target) && !this.Root.contains(FocusManager.get().currentFocus())) {
      this.toggleOpen(false);
    }
  }
  onInitialize() {
  ...
  toggleOpen(force) {
    ...
    this.isOpen = isOpen;
    if (this.isOpen) {
      FocusManager.get().setFocus(this.dropdownElements[this.selectedIndex] ?? this.Root);
      if (!this.listeningForClickOutside) {
        document.addEventListener("click", this.onClickOutsideEventListener);
        document.addEventListener("touchstart", this.onTouchOutsideEventListener);
  ...
  onItemSelected(index) {
    if (index < -1 || index >= this.dropdownItems.length) {
      throw new Error(
        `fxs-dropdown: invalid item index ${index}. Be sure to set dropdown-items before setting selected-item-index.`
      );
    }
  ...
  updateExistingElement(element, dropdownItem, isSelected) {
    ...
    if (audioGroup) {
      element.setAttribute("data-audio-group-ref", audioGroup + "-dropdown-item");
    }
  }
  update() {
  ...
  render() {
    ...
    this.Root.innerHTML = `
			<div class="dropdown__container no-pan w-full relative flex flex-auto flex-row items-center justify-between pl-4 pr-1\\.5 ${containerClass.match(/border/) ? "" : "border-2"} border-primary-1 ${containerClass} ">
				<div class="dropdown__bg absolute inset-px transition-opacity bg-primary-3 ${bgClass}"></div>	
				<div class="dropdown__highlight absolute -inset-0\\.5 fxs-dropdown-gradient	img-dropdown-box-focus opacity-0 transition-opacity group-hover\\:opacity-100 group-focus\\:opacity-100 group-pressed\\:opacity-100"></div>
    ...
    this.scrollableElement.classList.add(
      "z-1",
      "absolute",
      "inset-x-0",
      "top-full",
    ...
    this.scrollableElement.setAttribute("allow-mouse-panning", "true");
...
class FxsDropdownItemElement extends FxsActivatable {
  ...
  onAttributeChanged(name, oldValue, newValue) {
    switch (name) {
      case "data-label":
        this.labelElement.dataset.l10nId = newValue ?? void 0;
        break;
      case "data-selected":
  ...
  render() {
    ...
    this.highlightElement.classList.add(
      "absolute",
      "inset-0",
      "img-dropdown-focus",
      "group-focus\\:opacity-70",
      "group-hover\\:opacity-70",
      "group-pressed\\:opacity-70",
      "opacity-0",
      "transition-opacity"
    );
    this.arrowElement.classList.add("mr-0\\.5", "rotate-180", "img-selection-arrow");
    this.labelElement.classList.add("relative", "pr-2", "flex-1");
    this.Root.setAttribute("non-prior-focusable", "");
...
export { DropdownSelectionChangeEvent, DropdownSelectionChangeEventName, FxsDropdown, FxsDropdownItemElement };
//# sourceMappingURL=fxs-dropdown.js.map
