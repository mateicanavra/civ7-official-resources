...
class FxsDropdown extends FxsActivatable {
  ...
  onWindowInputEventListener = this.onWindowInput.bind(this);
  ...
  onWindowInput(event) {
    ...
    if (event.detail.name === "touch-touch") {
      if (event.detail.status === InputActionStatuses.FINISH) {
        const target = event.target;
        if (target instanceof Node && !this.Root.contains(target) && !target.contains(this.Root)) {
          this.toggleOpen(false);
        }
      }
    }
  ...
  onAttach() {
    ...
    this.Root.listenForWindowEvent("engine-input", this.onWindowInputEventListener);
...
export { DropdownSelectionChangeEventName as D, FxsDropdown as F, DropdownSelectionChangeEvent as a, FxsDropdownItemElement as b };
//# sourceMappingURL=fxs-dropdown.chunk.js.map
