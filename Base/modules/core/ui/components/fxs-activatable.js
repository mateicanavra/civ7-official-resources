...
import { useVfx } from '../../ui-next/services/ui-vfx-support.js';
...
class FxsActivatable extends Component {
  ...
  onActivatableMouseEnterEventListener = this.onActivatableMouseEnter.bind(this);
  ...
  onActivatableMouseEnter() {
    this.triggerUiVFX("focus");
    if (this.isSoundEnabled) {
      this.playSound("data-audio-focus", "data-audio-focus-ref");
    }
  }
  onActivatableMouseLeave() {
    if (document.activeElement == this.Root && FocusManager.get().currentFocus() != this.Root) {
      this.Root.blur();
    }
    this.triggerUiVFX("focuslost");
  }
  onActivatableFocus() {
    this.triggerUiVFX("focus");
    ...
    if (inputEvent.detail.name == "touch-touch") {
      ...
      if (this.isSoundEnabled) {
        if (this.disabled || this.shouldPlayErrorSound) {
          this.triggerUiVFX("error");
          this.playErrorPressedSound();
        } else {
          this.triggerUiVFX("press");
  ...
  triggerUiVFX(eventName) {
    const rect = this.Root.getBoundingClientRect();
    const vfx = useVfx(this.Root.getAttribute("data-vfx-path") ?? void 0);
    vfx(eventName, rect);
  }
  playActivateSound() {
  ...
  onAttach() {
    super.onAttach();
    this.Root.addEventListener("mouseenter", this.onActivatableMouseEnterEventListener);
  ...
  onDetach() {
    ...
    this.Root.removeEventListener("mouseenter", this.onActivatableMouseEnterEventListener);
    super.onDetach();
  }
  addOrRemoveNavHelpElement(parent, value) {
    ...
    } else if (this.navHelp?.isConnected) {
...
export { ActionActivateEvent, ActionActivateEventName, FxsActivatable, UpdateActivatableDisabledState, UpdateFromOperationResult, FxsActivatable as default };
//# sourceMappingURL=fxs-activatable.js.map
