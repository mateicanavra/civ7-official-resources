...
class FxsSlider extends ChangeNotificationComponent {
  ...
  onAttributeChanged(name, _oldValue, newValue) {
    switch (name) {
      case "value": {
        ...
        this.setValue(value);
        this.realizeThumb();
        this.updateArrowDisabledState(value);
        break;
      }
      case "disabled":
  ...
  updateArrowDisabledState(value) {
    const isDisabled = this.disabled;
    this.leftArrow.setAttribute("disabled", isDisabled || value <= this.min ? "true" : "false");
    this.rightArrow.setAttribute("disabled", isDisabled || value >= this.max ? "true" : "false");
  }
  updateDisabledState() {
    const isDisabled = this.disabled;
    this.Root.classList.toggle("opacity-50", isDisabled);
    this.Root.classList.toggle("cursor-not-allowed", isDisabled);
    this.bar.classList.toggle("cursor-not-allowed", isDisabled);
    this.bar.classList.toggle("cursor-pointer", !isDisabled);
    this.thumb.classList.toggle("cursor-not-allowed", isDisabled);
    this.thumb.classList.toggle("cursor-pointer", !isDisabled);
    this.updateArrowDisabledState(this.value);
  }
  realizeThumb() {
...
Controls.define("fxs-slider", {
  ...
  attributes: [{ name: "value" }, { name: "disabled" }],
  tabIndex: -1
});

export { FxsSlider };
//# sourceMappingURL=fxs-slider.js.map
