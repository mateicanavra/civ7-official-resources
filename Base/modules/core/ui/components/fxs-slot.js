import { NavigationHandlers } from '../input/navigation-handlers.js';
import { NavigationRule, Navigation } from '../input/navigation-support.js';
...
class FxsSlot extends Component {
  ...
  continuousCheckForInitialFocusHelper() {
    if (this.focusManager.currentFocus() != this.Root) {
      return;
    }
    ...
    if (this.initialFocus) {
      this.focusManager.setFocus(this.initialFocus);
  ...
  onFocus() {
    ...
    if (!this.Root.hasAttribute("ignore-prior-focus") && this.priorFocus != null && this.priorFocus.isConnected) {
      this.focusManager.setFocus(this.priorFocus);
  ...
  onFocusOut(event) {
    ...
    const currentFocus = this.focusManager.currentFocus();
  ...
  onChildrenChanged(mutationList, _observer) {
      ...
      mutationList.forEach((mutationRecord) => {
        if (mutationRecord.type == "childList") {
          if (mutationRecord.addedNodes.length > 0) {
            const wasFocused = this.initialFocus == this.focusManager.currentFocus();
          ...
          mutationRecord.removedNodes.forEach((node) => {
            if (node == this.focusManager.currentFocus()) {
              findNewFocus = true;
    ...
    if ((this.focusManager.currentFocus() == this.Root || findNewFocus) && this.initialFocus && this.priorFocus == null) {
      this.focusManager.setFocus(this.initialFocus);
      this.priorFocus = this.initialFocus;
  ...
  navigate(direction) {
    ...
    const focus = this.focusManager.getFocusChildOf(this.Root);
    if (focus == null) {
      return true;
    }
    if (focus.parentElement == null) {
      console.error(
        `fxsslot: Attempt to navigate focus but the current focus '${this.focusManager.toLogString(null)}' doesn't have a parent element! (Was it disconnected?) rule: '${rule}', direction: '${direction}'`
      );
      return true;
    }
  ...
  handleNavigation(navigationEvent) {
    ...
    if (this.Root.isSameNode(this.focusManager.currentFocus())) {
      return true;
    }
    if (!this.Root.contains(this.focusManager.currentFocus())) {
      return true;
    }
...
class FxsSpatialSlot extends FxsSlot {
  ...
  onElementUnfocused(event) {
    if (!event.detail.native && event.detail.nextElement) {
      this.focusManager.setFocus(event.detail.nextElement);
      event.preventDefault();
      event.stopImmediatePropagation();
    }
  }
...
class FxsSlotGroup extends FxsSlot {
  ...
  onMutationObserved(_records) {
    const selectedSlot = this.Root.getAttribute("selected-slot");
    ...
    if (this.Root.hasChildNodes()) {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        if (child.id === selectedSlotId) {
          ...
          const currentFocus = this.focusManager.currentFocus();
          if (currentFocus.isSameNode(this.Root) || this.Root.contains(currentFocus)) {
            if (!child.isSameNode(currentFocus) && !child.contains(currentFocus)) {
...
export { FxsHSlot, FxsSidePanel, FxsSlot, FxsSlotGroup, FxsSpatialSlot, FxsVSlot, isSlot };
//# sourceMappingURL=fxs-slot.js.map
