...
function parseConstructibleAdjacency(def) {
  ...
  if (def.AdjacentTerrain) {
    ...
    if (resourceName) {
      result = Locale.compose("LOC_UI_ADJACENCY_INFO_OBJECT", amount, yieldName, resourceName);
    }
  } else {
    console.warn(
...
function parseConstructibleAdjacencyNameOnly(def) {
  ...
  if (def.AdjacentTerrain) {
    ...
    const resourceName = GameInfo.Resources.lookup(def.AdjacentSpecificResource)?.Name;
    if (resourceName) {
      result = Locale.compose("LOC_UI_ADJACENCY_INFO_RESOURCE_SHORT", resourceName);
    }
  } else {
    console.warn(
