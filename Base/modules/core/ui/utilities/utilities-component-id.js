...
((ComponentID2) => {
  ...
  function toBitfield(componentID) {
    const owner = BigInt(componentID.owner & 65535);
    const type = BigInt(componentID.type & 65535);
    const id = BigInt(componentID.id >>> 0);
    return owner << 48n | type << 32n | id;
  }
  ComponentID2.toBitfield = toBitfield;
  function fromBitfield(bitfield) {
    const owner = Number(bitfield >> 48n & 0xffffn);
    const type = Number(bitfield >> 32n & 0xffffn);
    const id = Number(bitfield & 0xffffffffn);
    return {
  ...
  ComponentID2.typeToString = typeToString;
...
export { ComponentID };
//# sourceMappingURL=utilities-component-id.js.map
