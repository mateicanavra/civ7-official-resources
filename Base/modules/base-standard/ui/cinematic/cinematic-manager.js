...
class CinematicManagerImpl extends DisplayHandlerBase {
  ...
  startEndOfGameCinematic(victoryCinematicType, victoryName, location, quoteAudio) {
    let vfxStr = void 0;
  ...
  onWonderCompleted(data) {
    if (this.isLoadingCurtainOpen()) {
      console.log("cinematic-manager: don't add display requests onWonderCompleted with the loading curtain up");
      return;
    }
  ...
  onNaturalWonderRevealed(data) {
    if (this.isLoadingCurtainOpen()) {
      console.log(
        "cinematic-manager: don't add display requests onNaturalWonderRevealed with the loading curtain up"
      );
      return;
    }
  ...
  onRandomEventOccurred(data) {
    if (this.isLoadingCurtainOpen()) {
      console.log(
        "cinematic-manager: don't add display requests onRandomEventOccurred with the loading curtain up"
      );
      return;
    }
  ...
  onCityProjectCompleted(data) {
    ...
    if (completedProject.ProjectType == "PROJECT_CREWED_SPACE_FLIGHT_PREP") return;
    if (ContextManager.shouldShowModalEvent(GameContext.localPlayerID) && GameplayMap.getRevealedState(GameContext.localPlayerID, data.location.x, data.location.y) == RevealedStates.VISIBLE) {
