...
class ResourceAllocationModel {
  ...
  update() {
    ...
    playerCities.getCityIds().forEach((cityID) => {
      ...
      if (city) {
        ...
        if (cityResources && cityTrade) {
          ...
          const assignedResources = cityResources.getAssignedResources();
          {
            assignedResources.forEach((resource) => {
              ...
              let resourceCount = 0;
              assignedResources.forEach((resourceToCount) => {
              ...
              if (resourceDefinition) {
                ...
                const tooltipText = this.getResourceTooltip(resourceDefinition, originCity?.name);
          ...
          const countTreasureResources = cityResources.getNumTreasureFleetResources(false);
                  ...
                  if (localResourceDefinition) {
                    if (localResourceDefinition.ResourceClassType === "RESOURCECLASS_TREASURE") {
                      treasureResources.push({
                        ...
                        origin: city.name,
                        bonus: this.getResourceTooltip(localResourceDefinition, city.name),
          ...
          const yields = this.getCityYieldData(city);
    ...
    const treasureFleetIcon = UI.getIcon("RESOURCECLASS_TREASURE_FLEET");
    const treasureFleetTooltip = Locale.stylize("[N]" + Locale.compose("LOC_CAN_CREATE_TREASURE_FLEET"));
    const unassignedYieldsTooltip = ResourceAllocationModel.getUnassignedTooltip(playerResources);
    this.allResources.forEach((resourceDefinition, resourceValue) => {
      ...
      let tooltipText = this.getResourceTooltip(resourceDefinition, originCity?.name);
      ...
      if (!isEmpireResource && !isTreasureResource && unassignedYieldsTooltip) {
        tooltipText = tooltipText + unassignedYieldsTooltip;
      }
      ...
      if (isDistantAndTreasure) {
        tooltipText = tooltipText + treasureFleetTooltip;
      }
      const resourceEntry = {
        ...
        name: resourceDefinition.Name,
        classType: resourceDefinition.ResourceClassType,
        classTypeIcon: isDistantAndTreasure ? treasureFleetIcon : UI.getIcon(resourceDefinition.ResourceClassType),
        origin: originCity?.name ?? "",
        bonus: tooltipText,
        value: resourceValue,
        count: 0,
  ...
  getCityYieldData(city) {
    const cityYields = city.Yields;
    const yieldData = [];
    if (cityYields) {
      for (const y of GameInfo.Yields) {
        const value = cityYields.getYield(y.YieldType);
        yieldData.push({ type: y.YieldType, label: y.Name, value: Locale.toNumber(value, "0.0") });
      }
    }
    return yieldData;
  }
  static getUnassignedTooltip(playerResources) {
    let unassignedYieldsTooltip = "";
    GameInfo.Yields.forEach((yieldDefinition) => {
      const unassignedBonus = playerResources.getUnassignedResourceYieldBonus(yieldDefinition.$hash);
    ...
    return unassignedYieldsTooltip != "" ? Locale.stylize("[NN]" + Locale.compose("LOC_RESOURCE_UNASSIGNED_BONUSES") + unassignedYieldsTooltip) : void 0;
  }
  // There's a modest upper-bound for resource types and cities, only localize each string once
  resourceTooltipCache = /* @__PURE__ */ new Map();
  originCityTooltipCache = /* @__PURE__ */ new Map();
  getResourceTooltip(resource, city) {
    let tooltip = this.resourceTooltipCache.get(resource.$hash);
    if (!tooltip) {
      tooltip = Locale.stylize(
        "{1_Name: upper}[N]{2_Class}[N]{3_Tooltip}",
        resource.Name,
        Locale.compose(
          "LOC_RESOURCECLASS_TOOLTIP_NAME",
          Locale.compose("LOC_" + resource.ResourceClassType + "_NAME")
        ),
        resource.Tooltip
      );
      this.resourceTooltipCache.set(resource.$hash, tooltip);
    }
    if (city) {
      let origin = this.originCityTooltipCache.get(city);
      if (!origin) {
        origin = Locale.stylize("[N]" + Locale.compose("LOC_UI_RESOURCE_ORIGIN", city));
        this.originCityTooltipCache.set(city, origin);
      }
      tooltip += origin;
    }
    return tooltip;
...
//# sourceMappingURL=model-resource-allocation.js.map
