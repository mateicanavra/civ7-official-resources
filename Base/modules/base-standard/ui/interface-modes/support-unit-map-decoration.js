...
((UnitMapDecorationSupport2) => {
  ...
  ((OverlayGroups2) => {
    ...
    OverlayGroups2[OverlayGroups2["commandRadius"] = 2] = "commandRadius";
    OverlayGroups2[OverlayGroups2["dynamic"] = 3] = "dynamic";
    OverlayGroups2[OverlayGroups2["attackStart"] = 100] = "attackStart";
  ...
  class Instance {
    ...
    mode = 2 /* both */;
    // Unit movement range overlay style
    unitSelectedOverlayPossibleMovementStyle = {
      style: "MovementRange",
      primaryColor: HighlightColors.unitPossibleMovementShadow,
      secondaryColor: HighlightColors.unitPossibleMovement
    };
    // Unit zone of control overlay style
    unitSelectedOverlayZoCStyle = {
      style: "ZocBorder",
      primaryColor: HighlightColors.unitMovementZOC,
      secondaryColor: HighlightColors.unitMovementZOCShadow
    };
    // Attackable tiles overlay style
    unitSelectedOverlayAttackStyle = {
      style: "CombatBorder",
      primaryColor: HighlightColors.unitAttack,
      secondaryColor: HighlightColors.unitAttackShadow
    };
    // Commander unit radius overlay style
    unitSelectedCommandRadiusStyle = {
      style: "CommanderRadius",
      primaryColor: HighlightColors.unitCommanderRadius
    };
    unitSelectedOverlayMovementGroup = WorldUI.createOverlayGroup(
      "UnitSelectedOverlayGroup",
      OVERLAY_PRIORITY.UNIT_MOVEMENT_SKIRT,
      { x: 1, y: 1, z: 1 }
    );
    unitSelectedOverlayZoCGroup = WorldUI.createOverlayGroup(
      "UnitSelectedOverlayZoCGroup",
      OVERLAY_PRIORITY.UNIT_COMBAT,
      { x: 1, y: 1, z: 1 }
    );
    unitSelectedOverlayAttackGroup = WorldUI.createOverlayGroup(
      "UnitSelectedOverlayAttackGroup",
      OVERLAY_PRIORITY.UNIT_COMBAT,
      { x: 1, y: 1, z: 1 }
    );
    ...
    unitMovementOverlay = this.unitSelectedOverlayMovementGroup.addBorderOverlay(
    ...
    unitZoneOfControlOverlay = this.unitSelectedOverlayZoCGroup.addBorderOverlay(
      this.unitSelectedOverlayZoCStyle
    );
    unitAttackOverlay = this.unitSelectedOverlayAttackGroup.addBorderOverlay(
    ...
    activate(unitID, mode) {
      ...
      this.unitAttackOverlay.clear();
      this.unitZoneOfControlOverlay.clear();
      this.unitZoneOfControlOverlay.startAlphaAnimation(5);
      this.unitZoneOfControlOverlay.setAlphaAnimationRange(0.8, 1);
    ...
    updateRanges() {
      ...
      if (unitMovement && unitMovement.movementMovesRemaining > 0) {
        ...
        let cleanMovePlots = null;
        cleanMovePlots = movePlots.filter((plot) => !kAttackPlots.includes(plot));
        if (cleanMovePlots != null && cleanMovePlots.length > 0) {
          this.unitMovementOverlay.setPlotGroups(cleanMovePlots, 1 /* possibleMovement */);
          this.unitMovementOverlay.setGroupStyle(
            1 /* possibleMovement */,
            this.unitSelectedOverlayPossibleMovementStyle
          );
        }
        let dynamicPlotGroupCounter = 3 /* dynamic */;
        if (zocPlots != null && zocPlots.length > 0) {
          for (const plot of zocPlots) {
            this.unitZoneOfControlOverlay.setPlotGroups(plot, dynamicPlotGroupCounter);
            this.unitZoneOfControlOverlay.setGroupStyle(
      ...
      if (unit.isCommanderUnit) {
        ...
        if (commandRadiusPlots.length > 0) {
          this.commandRadiusOverlay.setPlotGroups(commandRadiusPlots, 2 /* commandRadius */);
...
export { UnitMapDecorationSupport };
//# sourceMappingURL=support-unit-map-decoration.js.map
