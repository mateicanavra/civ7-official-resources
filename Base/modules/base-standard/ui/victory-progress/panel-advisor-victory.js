...
import AdvisorProgress from './model-advisor-victory.js';
import styles from './panel-advisor-victory.scss.js';
...
class PanelAdvisorVictory extends Panel {
  ...
  updateRadioButton(prevIndex, currentIndex) {
    ...
    FocusManager.get().setFocus(this.radioButtons[currentIndex]);
  }
  /**
    ...
    switch (direction) {
      case InputNavigationAction.LEFT: {
        if (this.isTopFocused) {
          ...
          FocusManager.get().setFocus(this.rewardElemends[this.selectedRewardIndex]);
        }
        live = false;
        break;
      }
      ...
      case InputNavigationAction.UP: {
        if (!this.isTopFocused) {
          ...
          FocusManager.get().setFocus(this.radioButtons[this.carouselIndex]);
        }
        live = false;
        break;
      }
