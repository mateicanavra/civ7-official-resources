...
((TreeNavigation2) => {
  ...
  ((Horizontal2) => {
    ...
    function treeNavigation(navigationEvent) {
      ...
      const originElement = FocusManager.get().currentFocus();
      ...
      const originRowAttribute = originCard.closest("[row]")?.getAttribute("row") ?? null;
      const originColAttribute = originCard.closest("[column]")?.getAttribute("column") ?? null;
      ...
      for (let i = 0; i < cards.length; ++i) {
        ...
        const candidateRowAttribute = card.closest("[row]")?.getAttribute("row") ?? null;
        const candidateColAttribute = card.closest("[column]")?.getAttribute("column") ?? null;
...
export { LineDirection, ScaleTreeCardEvent, ScaleTreeCardEventName, TreeCardBase, TreeCardScaleBoundary, TreeClassSelector, TreeGridDirection, TreeNavigation, TreeNodesSupport, TreeSupport, UpdateLinesEvent, UpdateLinesEventName };
//# sourceMappingURL=tree-support.js.map
