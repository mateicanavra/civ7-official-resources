...
import '../scripts/voronoi_maps/unified-continents-base.js';
...
console.log("Generating using script small-continents-voronoi.ts");
...
async function generateMap() {
  ...
  const mapSettings = voronoiMap.getTypedSettings();
  mapSettings.totalPlayers = iTotalPlayers;
  voronoiMap.init(mapInfo.$index);
  const rules = voronoiMap.getGenerator().getRules();
  for (const value of Object.values(rules)) {
    for (const rule of value) {
      if (rule.name == RuleAvoidEdge.getName()) {
        rule.configValues.poleDistance = g_PolarWaterRows - 1;
      }
    }
  }
  voronoiMap.simulate();
  ...
  for (let y = 0; y < tiles.length; ++y) {
    for (let x = 0; x < tiles[y].length; ++x) {
      ...
      if (tile.isLand()) {
        ...
        if (tile.landmassId > 0 && tile.landmassId <= mapSettings.landmassCount) {
        ...
        if (tile.terrainType === TerrainType.Coast) {
          ...
          if (landmassId > 0 && tile.landmassId <= mapSettings.landmassCount) {
            TerrainBuilder.setLandmassRegionId(x, y, 1);
          } else {
            TerrainBuilder.addPlotTag(x, y, PlotTags.PLOT_TAG_ISLAND);
          }
